...
const ContactInfo = (props: IProps) => {
    ...
    return (
        <div className='border-b-[1px] border-bg-fc/10 pb-6 flex flex-col gap-6'>
            <h3
            ...
            <div
                className={`flex flex-col ${hover ? 'gap-6' : ''} cursor-pointer w-full group duration-500 pb-6`}
                onMouseEnter={() => {
                    isHover(true)
                }}
            ...
            >
                ...
                <div className={`relative duration-300 ${hover ? "h-12 pt-2 pb-3 px-2 opacity-100" : "opacity-0 h-0 "}`}>
                    <Link
                        href={link}
                        target='_blank'
                        className={
                            `absolute body-2-medium text-center top-0 w-full h-12 flex items-center justify-center gap-0
                                                            ...
                                                            duration-200 ${hover ? "change-background opacity-100 " : "bg-[rgba(252, 252, 252, 0.07)]"}`
                        }
                        style={{
                            borderRadius: 8,
                        }}
                    >
                        <span className={`${hover ? "text-black -translate-x-2 font-semibold" : "text-white translate-x-1"}  delayed-hover duration-300`}>{button}</span>
                        <svg
                            className={`${hover ? "opacity-100" : 'opacity-0'}  delayed-hover`}
                    ...
                    <div
                        className='absolute -bottom-12 w-full h-6'
                    ></div>
                </div>
            </div>
        </div>
